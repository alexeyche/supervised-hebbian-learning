...
opt = SGDOpt((0.001, 0.001, 0.001, 0.001))
opt.init(W, M, Wo, Mo)
...
for e in xrange(epochs):
	...
	for it in xrange(num_iters):
		...
		yo += dt * act(np.dot(y, Wo) - np.dot(yo, Mo))	
	...
	dMo = np.dot(yo.T, yo)/batch_size - np.eye(output_size)	
	
	opt.update(-dW, -dM, -dWo, -dMo)
	...
	for it in xrange(num_iters):
		yt += dt * (act(np.dot(xtv, W) - np.dot(yt, M)) - yt)
		yto += dt * act(np.dot(yt, Wo) - np.dot(yto, Mo))	
